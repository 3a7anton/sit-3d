...
export interface DepthCarouselProps {
  ...
  onExpand?: (item: DepthCarouselItem, index: number) => void;
}
...
export const DepthCarousel: React.FC<DepthCarouselProps> = ({
  ...
  className = '',
  activeIndex,
  onCardClick: externalOnCardClick,
  onExpand
}) => {
  ...
  // Synchronize with external activeIndex if controlled
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // Allow vertical page scroll if not horizontal wheeling
  ...
  const onPointerEnd = useCallback(() => {
    const drag = dragRef.current;
    if (!drag) return;
    if (rootRef.current && drag.moved && rootRef.current.hasPointerCapture?.(drag.id)) {
    ...
    const cfg = cfgRef.current;
    const stepPx = Math.max(cfg.cardWidth * 0.55 * scaleRef.current, 40);
    const projected = posRef.current - (v * 180) / stepPx;
  ...
  const onCardClick = useCallback(
    (index: number) => {
      ...
      if (index === focusRef.current) {
        externalOnCardClick?.(index, data[index]);
        onExpand?.(data[index], index);
      } else {
        setFocus(index, true);
      }
    },
    [setFocus, externalOnCardClick, onExpand, data]
  );
          ...
          return (
            ...
            >
              ...
              <div className="depth-carousel__card-body">
                {/* Top header row: Badge + Tag + Expand icon */}
                <div className="depth-carousel__card-top">
                  <div className="depth-carousel__card-badge-row">
                    {item.badge && <span className="depth-carousel__card-badge">{item.badge}</span>}
                    {item.tag && <span className="depth-carousel__card-tag">{item.tag}</span>}
                  </div>
                  {onExpand && (
                    <span className="depth-carousel__card-expand-icon" title="Click to expand">
                ...
                {/* Optional area icon */}
                {Icon && (
                  <div className="depth-carousel__card-icon-box">
                    <Icon className="w-5 h-5 text-amber-400" />
                  </div>
                )}

                {/* Main Content */}
                <div className="depth-carousel__card-main">
                  {item.title && <h3 className="depth-carousel__card-title">{item.title}</h3>}
                  {item.subtitle && <p className="depth-carousel__card-subtitle">{item.subtitle}</p>}
                </div>

                {/* Footer with Description & Expand action hint */}
                <div className="depth-carousel__card-footer">
                  {item.description && (
                    <p className="depth-carousel__card-desc">{item.description}</p>
                  )}
                  <div className="depth-carousel__card-action-hint">
                    <span>Click to expand details</span>
                    <span className="depth-carousel__card-arrow">→</span>
                  </div>
                </div>
              </div>
            </div>
